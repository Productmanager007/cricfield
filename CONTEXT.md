...
- **Builds today:** `web/`, MILESTONE Phases 1 (`6a8a4fb`), 2 (`d1eaaf2`), 3 (`433199d`) and 4 (`66807a1`). `npm run build` in `web/` produces a static export in `web/out/` with four routes. `/` is the fielding leaderboard: it fetches `/data/players.json` at runtime and shows all 528 qualified fielders, sortable by column and searchable by name. It sits at the site root because MILESTONE Phase 6 requires the site URL to serve the leaderboard. `/provenance` is the Phase 1 provenance page, moved there from `/`; it fetches `/data/meta.json` and renders match count, delivery count, season range and `git_commit`. `/player/<slug>` is statically generated for each of the 528 slugs in `seasons/index.json`, and every leaderboard name links to one. It fetches `/data/seasons/<slug>.json` for the season-by-season table, and reads the player's `players.json` row for the career line. Seasons with `qualifies: false` stay in the table, marked LOW SAMPLE. The low-sample threshold in the page text is `season_qualify_balls` read from `meta.json`, not hardcoded. The season rate is labelled raw and unshrunk, apart from the shrunk career rate; substitute-only seasons, which have no field time, show a dash for it. `/compare?a=<slug>&b=<slug>` sets two players side by side and is linkable: the query string carries the pair, so a comparison pastes into a post or a DM. `players.json` and `meta.json` load with the route. Picking a player fetches only that player's season file, so picking two fetches exactly two, and the address bar updates without a navigation. Each player's career line is the same component as the player page. Common seasons get two season tables and one bar chart of raw FRAA/100, with low-sample seasons marked as on the player page. The chart's axis is scaled to qualifying seasons; a low-sample bar past it is cut with a break mark, with its value printed. A pair with no common season gets a stated "No common seasons" result and no chart. The build runs `web/scripts/copy-data.mjs` first, which copies `web-data/` into `web/public/data/` and fails loudly — exit 1, naming the missing directory — when `web-data/` is absent. It then runs `next build`, then `web/scripts/flatten-segments.mjs` (see §4).
- **Build size:** 16.17 MB at `66807a1`, then 16.94 MB once the favicon went in. The 528 player pages are 14.7 MB of that, and almost none of it is player data: each page's files repeat the same ~6 KB React Server Components payload four times. Anything added to the root layout is paid four times per page — the two icon links alone added about 1.5 KB per player page.
...
- **Limitations note:** renders on every route from the root layout (`web/components/LimitationsNote.tsx`), in the static HTML, above any data. It names RA Jadeja and SA Yadav, and their ranks and the qualified-fielder count are read from `web/public/data` at build time (`web/lib/limitations.ts`). The build fails if either leaves the export, or ranks inside the top 10% — at that point the note's claim needs rewriting, not the data.
- **Absent:** methodology page and deploy — MILESTONE Phases 5–6, designed in `architecture.md` §14 and not started.
- **Untested:** everything. No test framework. The only standing checks are `scripts/data_quality.py` and the reconciliation built into `scripts/export_web.py`, which the export workflow runs and which fails the job on disagreement.
- **CI:** `.github/workflows/export.yml` on GitHub Actions is the canonical producer of `web-data/`. It runs the exporter on pushes to `main` touching `cricfield/`, `scripts/export_web.py` or `requirements.txt`, and on manual dispatch, and publishes the `web-data` artefact. It is canonical for provenance — a clean checkout records an accurate `git_commit` with `git_dirty_known: true` (`architecture.md` §14) — and it is also the only export path that works on this machine: Smart App Control blocks local Python from loading numpy's DLLs. **Do not run the export locally; download the artefact from the newest successful run.**
...
- **Next.js 16.3.4 writes segment-prefetch files under the wrong names on Windows.** `collectSegmentPathsImpl` in `next/dist/export/index.js` builds segment paths with `path.relative()`, so on Windows they contain `\`. `convertSegmentPathToStaticExportFilename` replaces only `/`, so the export writes `provenance/__next.provenance/__PAGE__.txt` where the router requests `provenance/__next.provenance.__PAGE__.txt`. Every link prefetch to a nested route was a 404. `web/scripts/flatten-segments.mjs` renames the files after `next build`; a Linux build writes flat names and the script does nothing. Remove it when Next normalises the separator. 16.3.4 is the latest stable release as of 2026-09-11.
