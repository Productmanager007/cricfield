...
- **Builds today:** `web/`, MILESTONE Phases 1 (`6a8a4fb`) and 2 (`d1eaaf2`). `npm run build` in `web/` produces a static export in `web/out/` with two routes. `/` is the fielding leaderboard: it fetches `/data/players.json` at runtime and shows all 528 qualified fielders, sortable by column and searchable by name. It sits at the site root because MILESTONE Phase 6 requires the site URL to serve the leaderboard. `/provenance` is the Phase 1 provenance page, moved there from `/`; it fetches `/data/meta.json` and renders match count, delivery count, season range and `git_commit`. The build runs `web/scripts/copy-data.mjs` first, which copies `web-data/` into `web/public/data/` and fails loudly — exit 1, naming the missing directory — when `web-data/` is absent.
...
- **Limitations note:** renders on every route from the root layout (`web/components/LimitationsNote.tsx`), in the static HTML, above any data. RA Jadeja's rank and the qualified-fielder count in it are read from `web/public/data` at build time (`web/lib/limitations.ts`). The build fails if he leaves the export, or if he ranks inside the top 10% — at that point the note's claim needs rewriting, not the data.
- **Absent:** player detail, compare, methodology page and deploy — MILESTONE Phases 3–6, designed in `architecture.md` §14 and not started.
